...
THEMES = {
    "🌑 Dark Cosmos": {
        ...
        "bg_secondary":  "#0f172a",
        "bg_card":       "#111827",
        "bg_sidebar":    "linear-gradient(170deg,#080b14 0%,#0f172a 100%)",
        "accent1":       "#6366f1",
        "accent2":       "#22d3ee",
        "accent3":       "#34d399",
        "accent4":       "#f59e0b",
    ...
    "☀️ Light Pro": {
        ...
        "bg_secondary":  "#ffffff",
        "bg_card":       "#ffffff",
        "bg_sidebar":    "linear-gradient(170deg,#e0e7ff 0%,#f8fafc 100%)",
        "accent1":       "#4f46e5",
        "accent2":       "#0891b2",
        "accent3":       "#059669",
        "accent4":       "#d97706",
    ...
    "⚡ Neon Cyber": {
        ...
        "bg_secondary":  "#050d1a",
        "bg_card":       "#060e1c",
        "bg_sidebar":    "linear-gradient(170deg,#020408 0%,#050d1a 100%)",
        "accent1":       "#00ff88",
        "accent2":       "#00cfff",
        "accent3":       "#ff0099",
        "accent4":       "#ffcc00",
...
if "theme" not in st.session_state:
    st.session_state.theme = "🌑 Dark Cosmos"

selected_theme = st.sidebar.selectbox(
    "🎨 Dashboard Theme", list(THEMES.keys()),
    index=list(THEMES.keys()).index(st.session_state.theme)
)
st.session_state.theme = selected_theme
T = THEMES[selected_theme]

ACCENT_COLORS  = [T["accent1"], T["accent2"], T["accent3"], T["accent4"], T["accent5"], "#a78bfa"]
...
def load_data(path) -> pd.DataFrame:
    ...
    if "Order Date" in df.columns and "Customer ID" in df.columns:
...
# ─────────────────────────────────────────────
# ── Sidebar: data source  ✅ FIXED SECTION ──
# ─────────────────────────────────────────────
...
<div style="background:{T['bg_secondary']};border:1px solid {rgba(T['accent1'], 0.27)};
  ...
  <div style="color:{T['text_primary']};font-size:0.85rem;line-height:1.5;">
    🖱️ <b>Drag & drop</b> your CSV file below, or click <b>Browse files</b>
  </div>
</div>
""", unsafe_allow_html=True)

# ✅ FIX: label_visibility="collapsed" lets the drop zone be the full clickable/draggable area
uploaded = st.sidebar.file_uploader(
    label="📁 Upload Superstore CSV",
    type=["csv"],
    label_visibility="collapsed",   # ← KEY FIX: removes the label so full area is drag-droppable
    accept_multiple_files=False,
    help="Drag a CSV file here or click Browse files to select from your computer"
)
...
if uploaded:
    ...
    st.sidebar.markdown('<div class="alert-success">✅ File uploaded successfully!</div>', unsafe_allow_html=True)
elif os.path.exists(DEFAULT_PATH):
    df = load_data(DEFAULT_PATH)
    st.sidebar.markdown('<div class="alert-info">📁 Using local dataset</div>', unsafe_allow_html=True)
else:
    st.sidebar.markdown('<div class="alert-danger">❌ No data found — upload a CSV above</div>', unsafe_allow_html=True)
    st.error("❌ No data found. Please drag & drop or upload a CSV file from the sidebar.")
    st.stop()
...
# ══════════════════════════════════════════
with tabs[6]:
    st.markdown('<div class="section-title">🤖 Ask AI From Your Data (Gemini)</div>', unsafe_allow_html=True)
    ...
    def ask_gemini(question: str, df_in: pd.DataFrame) -> str:
        ...
        try:
            ...
            prompt  = f"""You are a senior data analyst for a retail superstore.
    ...
    if st.button("🔍 Get AI Answer", type="primary"):
        if not user_q:
            ...
            <div style="background:{T['bg_card']};border-radius:16px;padding:22px;
                        border:1px solid {T['border']};color:{T['text_primary']};line-height:1.75;
                        font-size:0.9rem;">
            <b style="color:{T['accent1']}">🤖 AI Answer:</b><br><br>
